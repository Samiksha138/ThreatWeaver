---
description: "Threat modeling specialist. Use for security analysis, STRIDE assessments, threat identification, data flow diagrams, vulnerability assessment, and security architecture review."
...
When asked to publish, update the Confluence writeback page using `mcp_atlassian_updateConfluencePage` or REST API (`PUT /rest/api/content/{pageId}`).

**Cloud vs On-Prem:** Cloud URLs (`.atlassian.net`) use `/wiki` prefix; on-prem do not.
...
## Jira Integration
Create Jira issues for **Critical and High** severity threats only:
...
- **Description:**
  ...
  **Mitigations**
  ...
  - mitigation 2
  ```
- **Deduplication:** Check `{featureFolder}/jira-tickets.json` — skip if `threatName` already exists
- **Save to** `{featureFolder}/jira-tickets.json`:
  ```json
  [{"threatName": "...", "key": "SEC-123", "url": "https://jira.example.com/browse/SEC-123", "createdAt": "ISO-date"}]
  ```
- Requires Jira base URL, project key, and PAT configured in VS Code secrets
...
**⚠️ DO NOT create a "Products" or "products" parent folder. Product folders go DIRECTLY in the workspace root.**
...
- Jira tickets: `{featureFolder}/jira-tickets.json`
...
4. If insufficient context, note this limitation in `repo/threat-analysis.md` and rely on Confluence design specs for the primary analysis

**If no PRs are available:** Proceed with design-only analysis. Create `repo/threat-analysis.md` stating that code-level analysis was not possible.

## Knowledge Base
- Each product has a `{productFolder}/knowledge/` folder for lessons learned
- Always create this folder in Phase 0 (Preflight) if it doesn't exist
- After completing analysis (Phase 8 / Wrapup), save 1-3 lessons as `LESSON-XXX.md` files
- Lessons use YAML frontmatter with: title, domain, tags, product, feature, createdAt
